...
export async function handleCheckPhase(task, state, adapters) {
    ...
    await Promise.all(adapters.controls.map(async (control) => {
        ...
        const result = await runRecipe(adapters.tools.runner, await resolveProfile(adapters, task.type, control.checkRecipe.profile), control.checkRecipe, [{ phase, iteration, phaseState, controlState, otherPhases }], {
    ...
    const anyRaised = adapters.controls.some((c) => (updatedPhase.controls[c.name]?.raised?.length ?? 0) > 0);
    if (anyRaised) {
        const alreadyQueued = state.remainingTasks.some((t) => t.type === 'investigate-phase' && t.phase === phase);
        if (alreadyQueued)
            return { ...updatedState, remainingTasks: state.remainingTasks };
        const investigateTask = { type: 'investigate-phase', phase };
        return {
            ...updatedState,
