...
export async function revise(state, adapters, question, answer, options) {
    const result = await runRecipe(adapters.tools.runner, await resolveProfile(adapters, 'revise-spread'), { profile: '', prompt }, [
        {
            question,
            answer,
    ...
    const directPhases = Array.isArray(question.phaseIndex)
        ? question.phaseIndex
        : [question.phaseIndex];
    const additionalPhases = parsed.additionalPhases ?? [];
    const allAffected = [...new Set([...directPhases, ...additionalPhases])].sort((a, b) => a - b);
    const alreadyQueued = new Set(state.remainingTasks
    ...
    if (drainResult.status === 'needs-answers') {
        throw new Error('revise: unexpected needs-answers state — gather-questions should not run during revise');
    }
